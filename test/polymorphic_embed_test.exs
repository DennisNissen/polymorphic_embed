defmodule PolymorphicEmbedTest do
    ...
    test "validates lists of polymorphic embeds" do
      for generator <- @generators do
        ...
        if polymorphic?(generator) do
          ...
          assert %{address: {"can't be blank", [validation: :required]}} =
                   Map.new(location_errors)
        end
  ...
  describe "generate ID" do
    test "for single embed in data" do
      for generator <- @generators do
        reminder_module = get_module(Reminder, generator)
        sms_module = get_module(Channel.SMS, generator)
    ...
    test "for single embed in changes" do
      for generator <- @generators do
        reminder_module = get_module(Reminder, generator)
    ...
    test "for list of embeds in data" do
      for generator <- @generators do
        reminder_module = get_module(Reminder, generator)
    ...
    test "list of embeds in changes" do
      for generator <- @generators do
        reminder_module = get_module(Reminder, generator)
  ...
  describe "list of embeds" do
    ...
    test "defaults to [] after insert" do
      for generator <- @generators do
        reminder_module = get_module(Reminder, generator)
